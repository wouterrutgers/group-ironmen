...
const componentBuildPlugin = {
  name: 'componentBuild',
  setup(build) {
    const components = new Set(JSON.parse(fs.readFileSync('resources/components/components.json', 'utf8')));
...
const htmlBuildPlugin = {
  name: "htmlBuild",
  setup(build) {
    const components = JSON.parse(fs.readFileSync('resources/components/components.json', 'utf8'));
    ...
    build.onEnd(async () => {
      let htmlFile = await fs.promises.readFile("resources/components/index.html", "utf8");

      const cssFiles = ['resources/components/main.css', ...components.map((component) => `resources/components/${component}/${component}.css`)];
      const cssReadResults = await Promise.all(cssFiles.map((cssFile) => fs.promises.readFile(cssFile, "utf8")));
      let css = cssReadResults.join('');

      for (let imagePath of imagesToInline) {
      ...
      await fs.promises.writeFile("resources/views/index.blade.php", htmlFile);
    });
  }
};
...
function build() {
  esbuild.build({
    entryPoints: ['resources/components/index.js'],
    bundle: true,
    sourcemap: true,
    minify: false,
    format: 'esm',
...
if (watch) {
  const watcher = chokidar.watch('resources/components', {
